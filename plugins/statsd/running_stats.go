...
type RunningStats struct {
	...
	// We will store a maximum of percLength values, at which point we will start
...
func (rs *RunningStats) AddValue(v float64) {
	...
	if rs.n == 0 {
		...
		rs.perc = make([]float64, 0, rs.percLength)
	}
...
func (rs *RunningStats) Percentile(n int) float64 {
	...
	i := int(float64(len(rs.perc))*float64(n)/float64(100)) - 1
	if i < 0 {
		i = 0
	}
	return rs.perc[i]
